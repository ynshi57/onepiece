...
// lives here: voice-broadcast toggle, optional remote Qwen/Mac VQA, model picker,
// multi-backend picker, advanced server configuration, debug text, and hotspot help.
...
struct SettingsView: View {
            ...
            Form {
                ...
                Section("诊断上传") {
                    Toggle("上传诊断帧", isOn: $viewModel.isDiagnosticRecordingEnabled)
                        .disabled(!viewModel.isRemoteVQAEnabled)
                    Text(viewModel.diagnosticRecordingText)
                        .font(Theme.Typography.caption)
                        .foregroundStyle(.secondary)
                    Text(
                        viewModel.isRemoteVQAEnabled
                            ? "会把压缩画面和本地模型输出发送到当前连接的 Mac 后端，用于分析误检、漏检和 overlay 对齐。仅测试时开启。"
                            : "需要先打开「远程风险解释」并连上 Mac，才能上传诊断帧。"
                    )
                        .font(Theme.Typography.caption)
                        .foregroundStyle(.secondary)
                }
    ...
    @ViewBuilder
    private var remoteVQASections: some View {
        Section("语音与模型") {
            HStack {
                Text("本地模型")
                Spacer()
