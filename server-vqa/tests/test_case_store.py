...
def _grid(cells):
    return {"cols": 2, "rows": 2, "cells": cells}


def _row(fid, cells):
    return {"frame_id": fid, "ground_truth": {"traversable_grid": _grid(cells)}}


def _pred(fid, cells):
    return {"frame_id": fid, "prediction": {"traversable_grid": _grid(cells)}}


# --- classification -------------------------------------------------------


def test_region_miss_when_truth_walkable_but_pred_empty():
    gt = {"traversable_grid": _grid([1, 0, 0, 0])}
    pred = {"traversable_grid": _grid([0, 0, 0, 0])}
    assert frame_failure_types(gt, pred) == {"region_miss"}


def test_region_false_go_when_truth_empty_but_pred_walkable():
    gt = {"traversable_grid": _grid([0, 0, 0, 0])}
    pred = {"traversable_grid": _grid([1, 0, 0, 0])}
    assert frame_failure_types(gt, pred) == {"region_false_go"}
...
def test_case_classification_agrees_with_viewer_flags():
    gt = {"traversable_grid": _grid([1, 0, 0, 0])}
    pred = {"traversable_grid": _grid([0, 1, 0, 0])}
    case_types = frame_failure_types(gt, pred)
    ui_flags = _frame_flags(gt, pred)
    assert case_types == {"region_miss", "region_false_go"}
...
def test_cluster_groups_by_failure_type_with_deterministic_ids():
    ...
    assert set(by_type) == {"region_miss", "region_false_go"}
    assert by_type["region_miss"]["case_id"] == make_case_id("camvid-manifest", "region_miss")
    assert by_type["region_miss"]["frame_ids"] == ["a", "b"]
    assert by_type["region_miss"]["frame_count"] == 2
    assert by_type["region_false_go"]["frame_ids"] == ["c"]


def test_cluster_reads_row_root_traversable_grid():
    """Adapters emit the grid on the row, not inside ground_truth/prediction."""
    manifest = [{"frame_id": "a", "ground_truth": {}, "traversable_grid": _grid([1, 0, 0, 0])}]
    preds = {"a": {"frame_id": "a", "prediction": {}, "traversable_grid": _grid([0, 0, 0, 0])}}
    clusters = cluster_failures(manifest, preds, dataset_key="ds")
    assert [c["failure_type"] for c in clusters] == ["region_miss"]
    assert clusters[0]["frame_ids"] == ["a"]
...
def test_upsert_creates_then_updates_without_duplicating():
    manifest = [_row("a", [1, 0, 0, 0])]
    preds = {"a": _pred("a", [0, 0, 0, 0])}
    clusters = cluster_failures(manifest, preds, dataset_key="ds")
    ...
    manifest.append(_row("b", [1, 0, 0, 0]))
    preds["b"] = _pred("b", [0, 0, 0, 0])
