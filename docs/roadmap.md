...
把 VQASee 做成 iPhone 上**视觉优先**的风险辅助与通行路径提示：行走、骑行、驾驶时，用户能看见车道边界、按角色区分的可通行区、障碍，并且够快、敢信。不替代观察或驾驶责任。

## 当前阶段（2026-08-27 裁定 · 2026-09-13 复盘重申）

**只做户外街景四项核心，四项同为验收硬指标：**

1. **车道线**：车道线与道路边界，作为通行区和风险边界。
2. **可通行区域（必须区分人和车）**：行人偏人行道，机动车偏车道；同一画面两套可走，不是全局“可走”。
3. **障碍物**：人、车、台阶、路沿等及其方向。
4. **实时性**：以上三项在 iPhone 上实时可用；慢到不能实时等于没做。

验收口径：逐像素可走区 IoU + 引导线 + 角色区分 + 车道线 + 端上延迟。退役粗糙三区 ROI。

评测台、CamVid、12 帧 test、harness **只为这四项服务**，不是并列产品。Qwen 问答、读文字、周围模式、relay 增强，默认进 backlog，除非直接服务这四项或它们的失败可见。
...
> 2026-09-13：下列条目是历史队列。与四项核心冲突时，以「当前阶段」为准；读文字、Qwen 路由、周围模式 2s 默认 backlog。
