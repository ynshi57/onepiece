...
## 当前阶段（2026-09-22 拍板 · 见 `docs/CURRENT.md`）

**只做户外街景四项核心，四项同为验收硬指标。** 现行写法以 CURRENT 为准。

1. **车道线**：live 看 TwinLite 出的线/车道图；UFLDv2 是下一阶段。
2. **可通行区域（必须区分人和车）**：承诺仍在；live TwinLite 尚未兑现角色可走。
3. **障碍物**：人、车、台阶、路沿。台阶/路沿不是 YOLO 类。
4. **实时性**：以上三项在 iPhone 上实时可用。

评测台、CamVid、harness 只为这四项服务。Qwen 问答、读文字、周围模式默认以后再做。
...
> 2026-09-22：下列条目不是当前 P0。与四项核心冲突时以 `docs/CURRENT.md` 为准。
